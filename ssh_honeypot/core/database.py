...
class HoneyDB(DatabaseBackend):
    ...
    def _init_db(self):
        ...
        conn.execute("PRAGMA busy_timeout = 30000;") # Ensure persistent timeout setting
        c = conn.cursor()
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS session_summaries_cache (
        ...
        c.execute('CREATE INDEX IF NOT EXISTS idx_llm_usage_ip_time ON llm_usage(ip, timestamp)')
        ...
        c.execute('CREATE INDEX IF NOT EXISTS idx_interactions_session ON interactions(session_id)')
        c.execute('CREATE INDEX IF NOT EXISTS idx_interactions_md5 ON interactions(request_md5)')
    ...
    def log_auth_event(self, client_ip, username, auth_method, auth_data, success, client_version, fingerprint=None, protocol='ssh'):
        conn = None
        try:
            fp_json = "{}"
            if fingerprint:
            ...
            c.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (client_ip, username, auth_method, auth_data, success, client_version, fp_json, protocol))
            conn.commit()
        except Exception as e:
    ...
    def start_session(self, session_id, ip, username, password, client_version, fingerprint=None, protocol='ssh'):
        conn = self._get_conn()
        try:
            fp_json = "{}"
            if fingerprint:
    ...
    def log_interaction(self, session_id, cwd, command, response, source="unknown", was_cached=False, duration_ms=0, request_md5=None, response_md5=None, response_head=None, response_size=None):
        ...
        if response is not None:
             if response_size is None:
                 try: response_size = len(response)
                 except: pass
             
             if response_head is None:
                 try: response_head = str(response)[:100]
                 except: pass
    ...
    def update_user_file(self, ip, username, path, parent_path, type, metadata, content=None):
        ...
        if type == 'file' and self.is_managed_directory(ip, username, path):
             ...
             for item in self.skeleton_cache:
                 skel_path = item['path']
                 if skel_path.startswith('~'): skel_path = skel_path.replace('~', home_dir, 1)
                 if skel_path == path and item['type'] == 'directory':
                     is_known_dir = True
                     break
             
             if is_known_dir:
                 log.warning(f"[FS Guard] Prevented overwriting directory '{path}' with file content: {str(content)[:50]}")
    ...
    def record_llm_usage(self, ip, source='http'):
        conn = self._get_conn()
        try:
            conn.execute("INSERT INTO llm_usage (ip, source) VALUES (?, ?)", (ip, source))
            conn.commit()
        except Exception as e:
    ...
    def check_llm_rate_limit(self, ip, rpm_limit, rpd_limit):
        ...
        try:
            ...
            c.execute("SELECT COUNT(*) FROM llm_usage WHERE ip = ? AND timestamp > datetime('now', '-60 seconds')", (ip,))
            rpm_count = c.fetchone()[0]
            if rpm_count >= rpm_limit:
                return False, f"RPM Limit Exceeded ({rpm_count}/{rpm_limit})"

            # Check RPD (Last 24 hours)
            c.execute("SELECT COUNT(*) FROM llm_usage WHERE ip = ? AND timestamp > datetime('now', '-24 hours')", (ip,))
            ...
            return True, "Error check failed (Fail Open)" # Fail open to ensure service continuity? Or fail closed? 
    ...
    def list_user_dir(self, ip, username, parent_path):
        ...
        for item in self.skeleton_cache:
            ...
            if not check_parent: check_parent = '/'
        ...
        try:
            c = conn.cursor()
            c.execute("SELECT * FROM user_filesystem WHERE ip = ? AND username = ? AND parent_path = ?", (ip, username, check_parent))
            rows = c.fetchall()
            columns = [col[0] for col in c.description]
            for r in rows:
    ...
    def scan_and_repair_corruption(self, ip, username):
        ...
        try:
            ...
            c.execute("SELECT path, content FROM user_filesystem WHERE ip=? AND username=? AND type='file' AND is_deleted=0", (ip, username))
            ...
            for path, content in candidates:
                ...
                if is_skel_dir or is_global_dir:
                    ...
                    # Safer to nuke it.
                    log.warning(f"[FS Repair] Found corrupt FILE '{path}' shadowing a directory. Content: {str(content)[:50]}...")
                    c.execute("DELETE FROM user_filesystem WHERE ip=? AND username=? AND path=?", (ip, username, path))
                    repaired_count += 1
                    
            if repaired_count > 0:
                conn.commit()
                log.info(f"[FS Repair] Repaired {repaired_count} corrupt entries for {username}@{ip}")
    ...
    def validate_anti_harvesting(self, ip, username, password):
        ...
        try:
            ...
            if username in unique_users:
                 return False, f"Anti-Harvesting: Blocked {ip} for user '{username}' (Known user, new password denied)"
            ...
            # Rule 2: Hard Limit
            if count >= max_auth:
                return False, f"Anti-Harvesting: Blocked {ip} for user '{username}' (Limit Reached: {count})"
            
            # Rule 3: Probabilistic Rejection
            prob = count / float(max_auth)
            if random.random() < prob:
                 return False, f"Anti-Harvesting: Randomly blocked {ip} for user '{username}' (Prob: {prob:.2f})"
    ...
    def get_session_interactions(self, session_id):
        ...
        try:
             c = conn.cursor()
             c.execute("SELECT command FROM interactions WHERE session_id=? ORDER BY id ASC", (session_id,))
    ...
    def get_cached_session_summary(self, chain_hash):
        ...
        try:
            c = conn.cursor()
            c.execute("SELECT summary, risk_score FROM session_summaries_cache WHERE chain_hash=?", (chain_hash,))
            return c.fetchone()
        finally:
            conn.close()
    ...
    def sanitize_artifacts(self):
        ...
        try:
            ...
            if deleted_home_pollution > 0:
                log.info(f"[DB] Cleaned {deleted_home_pollution} polluted user files from Global FS")
    ...
    def inspect_dir(self, ip, username, directory):
        ...
        report.append(f"{'Filename':<30} | {'GlobalFS':<10} | {'Skeleton':<10} | {'TempFS':<10} | {'Result':<10}")
        report.append("-" * 85)
