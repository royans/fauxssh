...
def cleanup_loop(db_instance):
    ...
    while True:
        try:
            ...
            if deleted_items:
                log.info(
                    f"[Cleanup] Removed {len(deleted_items)} old upload records from DB."
                )
                    ...
                    if os.path.isfile(fp):
                        if os.path.getmtime(fp) < cutoff:
                            try:
                                ...
                                log.error(
                                    f"[Cleanup] Failed to remove payload {f}: {e}"
                                )
...
def analysis_loop(
    db_instance, llm_instance, alert_manager_instance=None, run_once=False
):
    ...
    while True:
        try:
            ...
            for cmd_row in commands:
                ...
                if analysis:
                    log.info(
                        f"[Analysis] Processed: {cmd_text[:30]}... -> {analysis.get('explanation')}"
                    )
                    ...
                    try:
                        ...
                        alert_manager_instance.check_risk_score(
                            session_id, ip, score, explanation
                        )
                    except Exception as e:
