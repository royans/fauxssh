...
def validate_active_persona(config):
    ...
    if "system" in persona:
        if not persona["system"].get("hostname"):
            errors.append("Missing system.hostname")

    if "prompts" in persona:
        if not persona["prompts"].get("system_prompt"):
            errors.append("Missing prompts.system_prompt")

    # 3. Access Control checks
    if "access_control" in persona:
        ac = persona["access_control"]
        if "allow_root" not in ac:
            errors.append("Missing access_control.allow_root")
