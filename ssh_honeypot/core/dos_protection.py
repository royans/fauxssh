...
class DoSProtector:
    def __init__(self, limit_rpm=1000, ban_time_sec=3600):
        ...
        self.tracking = defaultdict(lambda: {'count': 0, 'window_start': 0, 'banned_until': 0})
                ...
                with open(self.ban_file, 'r') as f:
                    ...
                    for ip, until in data.items():
                        if until > now:
                            ...
                            count += 1
                    if count > 0:
                        log.info(f"[DoS Protection] Loadded {count} active bans from persistence.")
    ...
    def is_allowed(self, ip, service_name="unknown"):
        ...
        if record['count'] > self.limit_rpm:
            ...
            log.warning(f"[DoS Protection] BANNED IP {ip} from {start_str} until {end_str} ({self.ban_time_sec}s). Reason: Exceeded {self.limit_rpm} RPM on {service_name}.")
            
            # Send Discord Alert
            try:
                from ssh_honeypot.core.alert_manager import AlertManager
                AlertManager().send_ban_alert(ip, self.ban_time_sec, f"Exceeded {self.limit_rpm} RPM on {service_name}")
            except Exception as e:
