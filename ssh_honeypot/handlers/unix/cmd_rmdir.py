    ...
    def handle(self, cmd, context):
        ...
        if target_path.startswith('-'):
            if len(parts) > 2:
                ...
                return "rmdir: flags not fully supported\n", {}, {'source': 'local', 'cached': False}

        cwd = context.get('cwd', '/')
        abs_path = resolve_path(cwd, target_path)
        
        # Permission check
        allowed = abs_path.startswith('/tmp/') or abs_path.startswith('/home/') or abs_path.startswith('/root/')
        if not allowed:
        ...
        if not self.db:
            return "Internal Error: DB not available\n", {}, {'source': 'local', 'cached': False}
        ...
        return "", {'file_modifications': [{'action': 'delete', 'path': abs_path}]}, {'source': 'local', 'cached': False}
