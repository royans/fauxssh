...
try:
    from ssh_honeypot.core.network import network_persona
except ImportError:
    # Fallback
    from ssh_honeypot.core.network import network_persona
    ...
    if target_iface:
        ...
        for block in blocks:
            if block.startswith(target_iface + ":"):
                return block + "\n"
        return (
            f"{target_iface}: error fetching interface information: Device not found\n"
        )
...
"""
    ...
    tcp_ports = [
        ...
        (
            f"{network_persona.ip_eth0}:22",
            f"{client_ip}:{attacker_port}",
            "ESTABLISHED",
        ),  # Established SSH
    ]

    udp_ports = [("0.0.0.0:68", "0.0.0.0:*", "")]  # DHCP
    ...
    if "t" in flags or not ("u" in flags or "x" in flags):  # Default includes TCP
        for local, foreign, state in tcp_ports:
            if state == "LISTEN":
                if show_listening:
                    output.append(
                        f"tcp        0      0 {local:<23} {foreign:<23} {state}"
                    )
            elif state == "ESTABLISHED":
                if show_established:
                    output.append(
                        f"tcp        0     64 {local:<23} {foreign:<23} {state}"
                    )
...
def handle_ping(args):
    ...
    for seq in range(1, count + 1):
        ...
        delay_ms = delay * 1000
        output.append(
            f"64 bytes from {resolved_ip}: icmp_seq={seq} ttl=64 time={delay_ms:.2f} ms"
        )
        received += 1
    ...
    loss = 0
    output.append(
        f"{count} packets transmitted, {received} received, {loss}% packet loss, time {int(total_time*1000)}ms"
    )
    min_t = (latency_base - latency_jitter) * 1000
    max_t = (latency_base + latency_jitter) * 1000
    avg_t = latency_base * 1000
    output.append(
        f"rtt min/avg/max/mdev = {min_t:.3f}/{avg_t:.3f}/{max_t:.3f}/{latency_jitter*1000:.3f} ms"
    )
