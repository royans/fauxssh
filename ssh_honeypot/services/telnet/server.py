        ...
        while True:
            ...
            if not self.buffer:
                try:
                    ...
                    if not data: return line_bytes.decode('utf-8', errors='ignore').strip()
                    self.buffer += data
                except OSError:
            ...
            if not self.buffer: continue # Should return on EOF loop above, but safety
            ...
            if char == b'\x08' or char == b'\x7f':
                if len(line_bytes) > 0:
                    ...
                    if echo: self.sock.sendall(b"\x08 \x08")
                continue
            ...
            # Normal Char
            line_bytes += char
            if echo: self.sock.sendall(char)
    ...
    try:
        ...
        while True:
            ...
            if text.startswith("SSH-"):
                log.warning(f"[Telnet] Protocol Mismatch: SSH Client detected from {ip} (Banner: {text.strip()}). Rejecting.")
                client_sock.close()
                return
            
            if not text: continue # Prompt again effectively
        ...
        if not is_safe:
            ...
            client_sock.sendall(b"\r\nLogin incorrect\r\n")
            client_sock.close()
            db.log_auth_event(ip, username, 'password', password, False, "telnet_simple", fingerprint="telnet", protocol='telnet')
            return

        db.log_auth_event(ip, username, 'password', password, True, "telnet_simple", fingerprint="telnet", protocol='telnet')
        db.start_session(session_id, ip, username, password, "telnet_client", fingerprint="telnet", protocol='telnet')
        ...
        if persona_config and ('cisco' in persona_config.get('system', {}).get('handler_type', '') or 'juniper' in persona_config.get('system', {}).get('handler_type', '')):
            is_network_device = True
            
        if not is_network_device:
            client_sock.sendall(f"Linux {hostname} 3.16.0-6-amd64 #1 SMP Debian 3.16.56-1+deb8u1 (2018-04-23) x86_64\r\n".encode())
            client_sock.sendall(f"The programs included with the Debian GNU/Linux system are free software.\r\n".encode())
            client_sock.sendall(f"Last login: {time.ctime()} from 10.0.0.5\r\n".encode())
        ...
        while True:
            ...
            while i < len(chunk):
                ...
                # Handle IAC
                if byte == IAC:
                    if i+2 < len(chunk) and chunk[i+1:i+2] in [DO, DONT, WILL, WONT]:
                        i += 3
                        continue
                    i += 1
                    continue
                ...
                if byte == b'\n': is_newline = True
                    ...
                    if i+1 < len(chunk) and chunk[i+1:i+2] == b'\n':
                        i += 1 # Consumed. Handled by next iter? No, we just skipped LF.
                ...
                if is_newline:
                    ...
                    if cmd == "clear":
                        ...
                        try:
                            ...
                            elif isinstance(res_obj, (list, tuple)) and len(res_obj) == 2:
                        ...
                        if updates:
                            ...
                            if updates.get('env'): env.update(updates['env'])
                        ...
                        if net_resp:
                            ...
                            client_sock.sendall(net_resp.encode('utf-8', errors='replace'))
                        
                        # Log
                        db.log_interaction(session_id, cwd, cmd, resp_text, 
                    ...
                    is_cisco = False
                    if 'cisco' in config.get_persona_by_name('cisco_ios').get('system', {}).get('handler_type', '') or (persona_config and 'cisco' in persona_config.get('system', {}).get('handler_type', '')):
                        is_cisco = True
                    ...
                    if is_cisco:
                        ...
                        prompt_char = '>'
                        if priv >= 15: prompt_char = '#'
                        
                        host_disp = env.get('hostname_override', hostname)
                        if conf: host_disp += "(config)"
    ...
    def runner():
        ...
        try:
            ...
            while True:
                ...
                t = threading.Thread(target=handle_telnet_session, args=(client, addr, db, llm))
                t.daemon = True
                t.start()
        except Exception as e:
