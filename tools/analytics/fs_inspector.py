    ...
    if tree_view:
        ...
        for key, files in grouped.items():
            ...
            for f in files:
                ...
                accessed = (
                    to_local_time(f["last_accessed"]) if f["last_accessed"] else "Never"
                )
        ...
        for r in rows:
            ...
            accessed_val = (
                to_local_time(r["last_accessed"]) if r["last_accessed"] else "-"
            )
            ...
            table.add_row(
                ...
                f"[{type_style}]{type_icon}[/{type_style}]",
                path_val,
                size_val,
                ts_val,
                accessed_val,
...
def delete_user_files(conn, ip=None, username=None, filepath=None, skip_confirm=False):
    ...
    count = len(rows)
    console.print(
        f"[bold red]WARNING:[/bold red] You are about to delete [bold]{count}[/bold] files."
    )
    if ip:
        console.print(f"  Filter IP: {ip}")
    if username:
        console.print(f"  Filter User: {username}")
    if filepath:
        console.print(f"  Target File: {filepath}")
    if not ip and not username and not filepath:
        console.print("  [bold red]Filter: ALL FILES (No filters applied!)[/bold red]")
...
def main():
    ...
    parser.add_argument(
        "--tree", action="store_true", help="Show as tree view (grouped by session)"
    )
    parser.add_argument(
        "--cat", help="Show content of a specific file (requires --ip and --user)"
    )
    parser.add_argument("--file", help="Alias for --cat")
    parser.add_argument("--delete", action="store_true", help="Delete matching files")
    parser.add_argument(
        "--yes", action="store_true", help="Skip confirmation for deletion"
    )
        ...
        if args.delete:
            ...
            if target_file and (not args.ip or not args.user):
                ...
                if not args.ip or not args.user:
                    console.print(
                        "[red]Error: Deleting a specific file requires --ip and --user for safety.[/red]"
                    )
                    sys.exit(1)
