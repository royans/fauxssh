...
def list_uploads():
    ...
    query = (
        "SELECT ip, username, path, content, created_at, metadata FROM user_filesystem"
    )
    params = []
    ...
    print(
        f"{'Time':<20} | {'IP':<15} | {'User':<10} | {'Path':<30} | {'SHA256 (Preview)'}"
    )
    print("-" * 110)
    ...
    for r in rows:
        ...
        print(
            f"{created_at:<20} | {ip:<15} | {user:<10} | {path:<30} | {content_hash[:16]}..."
        )
...
def export_upload(ip, user, path, output_file):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT content FROM user_filesystem WHERE ip=? AND username=? AND path=?",
        (ip, user, path),
    )
    row = c.fetchone()
    conn.close()
...
def main():
    ...
    parser.add_argument(
        "--export",
        nargs=3,
        metavar=("IP", "USER", "PATH"),
        help="Export a specific file content",
    )
    parser.add_argument(
        "--out",
        type=str,
        default="exported_malware.bin",
