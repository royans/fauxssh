...
def db(tmp_path):
    ...
    with patch(
        "ssh_honeypot.core.fs_seeder.get_skeleton_data", return_value=MOCK_SKELETON
    ):
...
def test_last_accessed_update(db):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT last_accessed FROM user_filesystem WHERE ip=? AND path=?", (ip, path)
    )
...
def test_prune_uploads_aggressive(db):
    ...
    def inject_file(path, created_offset_days, accessed_offset_days=None):
        db.update_user_file(
            ip, user, path, "/home/alice", "file", {}, f"content_{path}"
        )
        conn = db._get_conn()

        created_ts = (
            datetime.datetime.now() - datetime.timedelta(days=created_offset_days)
        ).isoformat()

        if accessed_offset_days is not None:
            accessed_ts = (
                datetime.datetime.now() - datetime.timedelta(days=accessed_offset_days)
            ).isoformat()
            conn.execute(
                "UPDATE user_filesystem SET created_at=?, last_accessed=? WHERE path=?",
                (created_ts, accessed_ts, path),
            )
        else:
            conn.execute(
                "UPDATE user_filesystem SET created_at=?, last_accessed=NULL WHERE path=?",
                (created_ts, path),
            )
    ...
    inject_file(
        "/home/alice/abandoned.txt", 10, None
    )  # Created 10d ago, Never accessed
...
def test_prune_skips_tombstones(db):
    ...
    conn.execute(
        "UPDATE user_filesystem SET last_accessed=?, created_at=? WHERE path=?",
        (old_ts, old_ts, path),
    )
    conn.commit()
    conn.close()
...
def test_list_user_dir_includes_global_and_hides_tombstones(db):
    ...
    conn = db._get_conn()
    conn.execute(
        "INSERT OR REPLACE INTO global_filesystem (path, parent_path, type) VALUES (?, ?, ?)",
