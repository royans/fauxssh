...
class TestPersonaConfig:
    ...
    def config_mgr(self):
        ...
        if old_env: del os.environ["SSH_PERSONA"]
        
        yield ConfigManager()
        
        # Restore env
        if old_env: os.environ["SSH_PERSONA"] = old_env
        else: 
            if "SSH_PERSONA" in os.environ: del os.environ["SSH_PERSONA"]

    def test_load_persona_default(self, config_mgr):
        # Ensure StateManager returns None
        with patch('ssh_honeypot.core.state_manager.StateManager.get_last_persona', return_value=None):
            with patch.object(config_mgr, '_read_persona_file') as mock_read:
                # Mock base read
                mock_read.side_effect = lambda name, _: {"name": name} if name == "CentOS7_Legacy_Compute" else None
    ...
    def test_load_persona_override(self, config_mgr):
        ...
        with patch('ssh_honeypot.core.state_manager.StateManager.get_last_persona', return_value="state_p"):
