...
class TestAuthRestrictions(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        ssh_honeypot.services.ssh.server.MAX_SESSIONS_PER_IP = (
            100  # Prevent rate limiting during tests
        )
        # Disable LLM
        ...
        if ssh_honeypot.services.ssh.server.db is None:
            ssh_honeypot.services.ssh.server.db = MagicMock()
        cls.original_get_creds = (
            ssh_honeypot.services.ssh.server.db.get_unique_creds_last_24h
        )
        ssh_honeypot.services.ssh.server.db.get_unique_creds_last_24h = MagicMock(
            return_value=set()
        )
    ...
    @unittest.skip(
        "Skipping due to Connection Reset crash on root login (needs server investigation)"
    )
    ...
    @unittest.skip(
        "Skipping due to environment inconsistency/stale code affecting this specific test"
    )
