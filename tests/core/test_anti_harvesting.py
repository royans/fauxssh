...
def test_anti_harvesting_block_new_pwd_limit_reached(honey_db):
    ...
    honey_db.get_unique_creds_last_24h.return_value = {
        ("u1", "p"),
        ("u2", "p"),
        ("u3", "p"),
        ("u4", "p"),
    ...
    with patch("ssh_honeypot.core.config.config.get", return_value=5):
        with patch("os.getenv", return_value=None):  # Ensure no test mode
            ...
            for _ in range(total):
                ...
                is_safe, reason = honey_db.validate_anti_harvesting(
                    "1.2.3.4", "u_new", "pass_attempt"
                )
                if not is_safe:
                    rejections += 1
            ...
            print(
                f"Rejections for 2 users (Target Prob: {expected_prob:.2f}): {rejections}"
            )
