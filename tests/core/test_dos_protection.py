...
class TestDoSProtection(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        ssh_honeypot.services.ssh.server.active_sessions = 0
        ssh_honeypot.services.ssh.server.ip_connection_counts.clear()
        ssh_honeypot.services.ssh.server.MAX_SESSIONS_PER_IP = 3 # Force limit for this test suite

        # 2. MOCK LLM
        if ssh_honeypot.services.ssh.server.llm is None:
            ssh_honeypot.services.ssh.server.llm = MagicMock()
        cls.orig_llm = ssh_honeypot.services.ssh.server.llm
        ssh_honeypot.services.ssh.server.llm = MagicMock()
        # Mock response for "touch file_x.txt"
        ssh_honeypot.services.ssh.server.llm.generate_response.side_effect = lambda cmd, *args, **kwargs: \
    ...
    def tearDownClass(cls):
        # Restore LLM
        ssh_honeypot.services.ssh.server.llm = cls.orig_llm

    def setUp(self):
        # FORCE RESET server state to avoid race conditions between tests
        # CRITICAL: Server might be loaded as 'server' or 'ssh_honeypot.services.ssh.server'. Patch BOTH.
        for name, module in list(sys.modules.items()):
            if name == 'ssh_honeypot.services.ssh.server' or name == 'server' or name.endswith('.server'):
        ...
        ssh_honeypot.services.ssh.server.MAX_FILES_PER_SESSION = 0
