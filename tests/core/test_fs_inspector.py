...
def test_delete_specific_file(mock_db, capsys):
    ...
    cursor.execute(
        "SELECT count(*) FROM user_filesystem WHERE ip='192.168.1.100' AND path='/tmp/malware.sh'"
    )
    assert cursor.fetchone()[0] == 1
    # Secrets gone
    cursor.execute(
        "SELECT count(*) FROM user_filesystem WHERE ip='192.168.1.100' AND path='/root/secrets.txt'"
    )
    assert cursor.fetchone()[0] == 0
    conn.close()
