...
class TestEchoRedirect(unittest.TestCase):
    def setUp(self):
        ...
        self.context = {"cwd": "/home/test", "client_ip": "1.2.3.4", "user": "testuser"}
    ...
    def test_echo_redirect(self):
        ...
        if "> outfile.txt" in out:
            print(
                "FAIL: output contains redirection usage, means it was echoed literally."
            )
        else:
        ...
        out_cat, updates_cat, meta_cat = self.handler.process_command(
            "cat outfile.txt", self.context
        )
