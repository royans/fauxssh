...
def test_http_get_cache_miss(mock_server):
    ...
    with patch('ssh_honeypot.services.http_server.server.config') as mock_conf:
        ...
        mock_server.llm_interface.generate_response.return_value = "<html>New Content</html>"
        ...
        # Verify Cache Set
        mock_server.honey_db.cache_response.assert_called_with("HTTP GET /index.html", "HTTP_ROOT", "<html>New Content</html>")
...
def test_http_post_login(mock_server):
    ...
    with patch('ssh_honeypot.services.http_server.server.config') as mock_conf:
        ...
        mock_server.honey_db.get_cached_response.return_value = None
        mock_server.llm_interface.generate_response.return_value = "<form>Bad Creds</form>"
...
def test_http_llm_error_fallback(mock_server):
    ...
    with patch('ssh_honeypot.services.http_server.server.config') as mock_conf:
        ...
        assert b"200 OK" in output # We verify that we still return 200 OK but with error body (soft fail)
