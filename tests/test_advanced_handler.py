...
class TestAdvancedHandler:
    ...
    def test_head_file(self, handler):
        ...
        with patch.object(handler, '_generate_or_get_content', return_value=("LineA\nLineB\nLineC\n", "local")) as mock_get:
             out, _, _ = handler.process_command("head -2 foo.txt", {'cwd': '/'})
             assert out == "LineA\nLineB\n"
             mock_get.assert_called_with("head", "foo.txt", {'cwd': '/', 'env': {}})
    ...
    def test_debug_commands_access(self, handler):
        # Trusted IP
        with patch('ssh_honeypot.command_handler.get_ignored_ips', return_value=['1.2.3.4']):
